...
impl<'a> MaskedSocketBuffer<'a> {
    ...
    pub fn get_transmittable(&self, offset: usize, size: usize) -> Vec<u8> {
        ...
        if size > 0 {
            ...
            println!("get_transmittable: offset={}, size={}, buffer_len={}, mask_len={}", 
                    offset, size, self.buffer.len(), self.mask.len());
            println!("  data: {:?}", std::str::from_utf8(data).unwrap_or("<non-utf8>"));
            println!("  mask: {:?}", mask_slice);
            println!("  result: {:?}", std::str::from_utf8(&result).unwrap_or("<non-utf8>"));
        }
...
mod tests {
    ...
    fn test_reset_mask_range() {
        ...
        // Verify mask state
        assert_eq!(buffer.get_transmittable(0, 10), b"abghij");
        ...
        assert_eq!(buffer.get_transmittable(0, 10), b"aj");
        
        // Reset only middle portion
        buffer.reset_mask_range(3, 4); // Reset "defg"
        assert_eq!(buffer.get_transmittable(0, 10), b"adefgj");
        
        // Test reset beyond buffer length (should not crash)
        buffer.reset_mask_range(5, 20); // Beyond buffer size
        assert_eq!(buffer.get_transmittable(0, 10), b"adefghij");
    }
}
